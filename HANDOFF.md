...
**FIXED — search:**
- **Ranking regression** (`text_search.dart` `_orderedMatchFraction`, from `cdf6c6c`). Documented as "longest in-order run", implemented as a leftmost-greedy `indexOf(term, from)` whose cursor never rewinds — one query term whose FIRST doc occurrence sits late poisons the cursor past the rest of the doc. Measured on the real 12,370-doc hadith corpus: for an isnād query the correct matn `muslim:3878` scored 0.182 against a true 0.909 (1.07× boost) while a wrong doc took 3.0× — **correct hadith fell from rank 1 under the old plain BM25 to rank 40.** Now a true LCS (rolling 1-D DP), O(query × doc terms), non-query doc words skipped.
- **`toggleMic` cleared `results` but not `query`** (`voice_search_list_mixin.dart`, from `0b547da`). Every mic tap after a prior search rendered "No matches — clear the search" + "0 results" over an EMPTY field (which shows no X button to obey that message with) — transient ~2s normally, **permanent** when the recording decoded to nothing (mic denied / silence), recoverable only by switching tabs. New `test/voice_search_mixin_test.dart` is the FIRST widget test over the shared search shell.

**FIXED — matcher (`phoneme_matcher.dart`), the big one:**
- **The scoring window could exclude the very word the frontier was waiting on.** `wHi = min(reachHi, use.word + 6)`: `use.word` is where the tail's alignment STARTS, and a 24-token tail spans more than 6 words once words are short (2-4 phonemes) — so the frontier sat at `_reached` forever waiting on a word that was never scored (`frac[r+1]=0.00` in the ATOM trace). Now `wHi = min(reachHi, max(use.word + 6, _reached + 1))`.
  - **Corpus sweep, all 260 du'ās, perfect self-fed recitation: 29 failures → 4.** 5644/5703 words green (98.97%).
  - **Qur'an is byte-identical before/after** (surah 1 29/29, surah 112 13/14, Kursi follow-anywhere 46/46, sidestep 45/46) — the change only binds on FLAT clips (du'ā/hadith), where the frontier can outrun the localizer. Long hadith clips are the same shape, so they should benefit too — unverified, check on device.
- **One-word clips could never anchor.** The lock needs two consecutive greens, impossible at `wordCount == 1`, and skip-advance was unreachable too (`_stuckChunks` resets while `_anchor < 0`). `hisn-11` (غفرانك) and `hisn-142` (الأذان) were dead in follow-along; now anchor + green.
- **The score floor was unreachable for very short clips.** The localizer adds `kLocMatch` (3) per matched phoneme, so `hisn-197` (ولك, 3 phonemes) topped out at 9 against a flat `_scoreFloor` of 12 — the matcher rejected a PERFECT recitation of it forever. `_floor` is now `min(_scoreFloor, _ref.length * kLocMatch)`, which by construction only differs for clips under 4 phonemes — exactly one in the whole corpus. `hisn-197` now anchors and greens (`loc=0/9.0`).

**FIXED — UI + tests:**
- **Reveal chevrons rendered at 41% size** in BOTH footers (from `d6d1e14`). `Flexible(flex:1)` sat next to the existing `Spacer()` (an `Expanded(flex:1)`), so the `FittedBox` only ever got half the free width. The `Spacer` is now dropped when the reveal row is present: measured at 360×800, chevrons go **19×14 → ~38×28** (natural is 46×34; at 360dp the row genuinely cannot fit all four full-size, so some scale-down is unavoidable). New `test/reading_footer_layout_test.dart` measures the on-screen RECT — note `FittedBox` scales by paint transform, so the RenderBox size stays 46×34 and `element.size` cannot see this bug.
- **The du'ā anchor test guarded 6 of 260 openable du'ās** (from `6e94ef0`) — it iterated the `duas` const in `lib/data/duas.dart`, which NO screen reads, while the Azkar tab browses all 260 corpus entries. `dua_clip_test.dart` now sweeps the real browsable list with a named `knownIncomplete` allowlist, so any NEW failure breaks the build and any repair forces the list to shrink.
- **The sidestep comparison test's two real-surah groups had no assertions** — `f31ae02` shipped `allowBackward: false` into the Qur'an reader with zero enforced real-data coverage. They now assert anchor + green floors (85% follow-anywhere, 80% sidestep). Mutation-proven: the reviewer's `reachLo +1 → +9` mutation drops sidestep to 3/14 and 2/46 and now FAILS the suite (it passed before).

**ALSO FIXED — bookmarks + overflow menu** (was the last open finding): `_HeaderMenu` sits behind a four-way non-null guard no test satisfied. New `test/header_menu_test.dart` (9 tests) pins the gate, both sheets, their empty copy, the tap-to-open pop-then-open ordering, the remove button, history having NO remove button, and `BookmarkStar`. Three mutations were used to prove it bites: breaking the empty copy, dropping the `onRemove` call, and flipping the guard's `&&` to `||` (that last one needed an extra partial-inputs test — the first version of the suite missed it). Logging added to all three tabs: `_toggleBookmark` now logs `bookmark +key -> N saved` / `-key`, and each `onOpenEntry` logs `menu open <key>` under its own tab tag (`quranlist`/`dualist`/`hadithsearch`), so a device trace of a lost bookmark shows something.

**KNOWN-INCOMPLETE du'ās — down to 3, all diagnosed, none worth fixing** (pinned by name in `dua_clip_test.dart`'s `knownIncomplete`, so the list can only shrink):
- `hisn-31` (22/31 green), `hisn-53` (30/34), `hisn-74` (98/137) — **repeated-phrase du'ās** (hisn-31 is الله أكبر ×3, every phrase repeated 3×). Traced: the localizer correctly finds the true position (`loc=25`) but it sits outside the forward window (`win=[0,20]`), so `accepted=false`; skip-advance then walks the frontier to the end 1 word per 8 stuck chunks, but the later occurrences are credited to the FIRST copy so those words mark SKIPPED instead of green. Convergence measured out to 400 idle chunks — they plateau, they do not recover. **This is the documented backward-false-green family — proven unfixable in the matcher, do NOT re-attempt.**
- `hisn-197` is no longer in the failing set for real input; it stays in `knownIncomplete` only because the sweep harness feeds its 3 reference tokens exactly, below the matcher's separate "enough audio to localize" gate (`tail >= 6`). A device stream clears that instantly; a dedicated test pins that it anchors and greens on a realistic stream.

**Refuted by the verifiers (do not re-raise):** "live search query freezes mid-recitation" (the floor logic is correct); "every Qur'an matcher test still uses `allowBackward: true`" (f31ae02's judging mechanism is live device recitation, which DOES run sidestep); `corpus.json.gz` shrinking to 130 bytes (git-LFS pointer — `.gitattributes` now tracks `*.gz`).

**Papercuts found while testing (not fixed):**
- `Log`'s flush `Timer.periodic` (`util/log.dart:88`) is created lazily on the first line and never cancelled, so whichever widget test logs first fails teardown with "A Timer is still pending". `voice_search_mixin_test.dart` and `reading_footer_layout_test.dart` work around it with `setUpAll(() => Log.d(...))` to create it outside a fake-async zone. A real fix means a cancel hook, i.e. test-only API in production code.
- Two traps for anyone writing footer layout tests: `tester.binding.setSurfaceSize` silently did NOT apply (use `tester.view.physicalSize` + `devicePixelRatio`), and `FittedBox` scales by PAINT TRANSFORM — the RenderBox size stays 46×34 no matter how hard it is squeezed, so `element.size` cannot see a squeeze at all. Measure `tester.getRect`.

## SESSION 11 (2026-07-20) — CORE APP: BACKWARD-BUG VERDICT + REVIEW FIXES + VOICE TASBĪḤ COUNTER
...
**⭐ THE BACKWARD FALSE-GREEN IS PROVEN UNFIXABLE IN THE MATCHER — DEFERRED (do NOT re-attempt a matcher/threshold fix).** Built a host reproduction (`phoneme_matcher.dart`): re-reading a phrase whose word repeats ~1-2 words ahead false-greens the FORWARD copy. Fed two scenarios — (A) recite→w4 then re-read w0,w1 (backward), (B) recite straight forward through the repeat to w6 — and they produce a **byte-identical cumulative phoneme token stream** with **identical matcher state** (`reached=6 head=6 loc=0` both). So "I went back" and "I moved forward through the repeat" ARE the same input; every prior matcher-only fix failed because they tried to separate two identical inputs. `loc`(localizer bestStart)=0 in BOTH → the `head`-vs-`loc` margin idea is dead. A real fix needs a signal the matcher lacks: **(1) audio-gap** — RMS/silence discontinuity at the backward jump, fed from the mic layer to withhold forward advance (device-tuned threshold); or **(2) commit-horizon with rollback** — tentatively green, roll back if the reciter doubles back (A/B diverge only AFTER the identical prefix), costs marker latency + un-greening. Both device-tuned. **User first leaned "skip/remove backward," then explicitly DROPPED the removal idea ("leave the backward idea of removal")** — so backward tracking stays as-is; the false-green is a documented KNOWN LIMITATION (only fires on re-reads of nearby-repeating phrases; a synthetic even STUCK rather than self-healing, so it's not provably harmless). Full proof in memory `[[asr-matcher-cursor-contracts]]`. NOTE: a forward-only variant was measured worse before (Kursi 24/46→8/46), reinforcing "leave it."

**FIXED this session — commit `f5f106d` (3 review-found bugs in session-9/10 host-only-tested code + a mapping guard; 200 tests, analyze clean):**
- **Voice-search commit-horizon desync** (`voice_search_state.dart` `commitStablePrefix`). It appended `cur[i]` by index without checking `cur` still BEGINS with the committed prefix. The offline word model re-decodes the whole buffer each interim, so CTC can revise/insert/delete an EARLIER word → the committed query grew into a sequence no decode produced (fed BM25). Now HOLDS the prefix when a decode no longer starts with it; resumes on re-agreement, never un-commits. +2 tests (`commit_horizon_test.dart`).
- **125MB word model left resident on cancel-during-load** (`voice_search_state.dart` `start()`). `_handOffToFollowAlong` bails on `!_word.loaded`, so a cancel in the ~1s `ensureLoaded()` window skipped dispose+invalidate and the model sat resident through reading. Now freed in the post-load abort path, GUARDED (`!_busy && !_recording`) so a legitimate restart keeps it.
- **Quran reader kept a prior session's red "skipped" washes** (`reading_state.dart` `_loadClipAndMatcher`). Cleared `_asrRead` but not `_asrSkipped`, so restarting follow-along wiped green but left the previous attempt's red highlights until the first recognized chunk. One-liner clear — matches hadith/du'ā `_resetFollow`.
- **Muqaṭṭaʿāt mapping VERIFIED + pinned:** corpus→mushaf mapping sane across ALL 114 surahs (align validation report: 6236/6236 complete, 0 wrong; the old "49 incomplete" is stale). New full-corpus test `corpus→mushaf mapping is sane for ALL 114 surahs`.

**Reviews CLEARED (not bugs):** matcher hard-stall recovery is sound (flat-clip gate correct, `_reached` monotonic+bounded, self-heal works); hadith/du'ā reading states clean (`_disposed` discipline, balanced mic); hadith reader has no dangling `_onFinder`; unguarded `_wordPhonemes[..][0]` index is UNREACHABLE (0 empty-phoneme words across all 114 surahs + 259 du'ās + 12,370 hadith).

**NOTED, deliberately NOT fixed (user's call):** (a) a manual mic-STOP runs one more confidence update that could auto-open a reader on a stop-to-review gesture — debatable UX, not a clear bug; (b) `MicOwnership.claim` has a structural interleave race but no reproducible trigger (call pattern is release-then-claim) → guarding it = defensive code for a can't-happen case (style.md).

**NEW FEATURE — VOICE TASBĪḤ COUNTER on its own tab (commit `ea9b8fc`, reviewed, 207 tests, analyze clean).** User wanted the counter separated from the morning/evening adhkār and made voice-driven (dynamic, no per-phrase button — "whatever the reader says, just count it"). Built:
- **Counter = a 5th nav tab** (fingerprint icon), scoped to the short tasbīḥ phrases (سبحان الله · الحمد لله · الله أكبر · tahlīl · istighfār · salawāt). Morning/Evening protection adhkār stay in the browse `AdzkarScreen` (reachable from Home's suggestion card). Home's "Counter" tile now switches to the tab.
- **Voice counting:** footer mic (same `MicToggleButton`/`HearingIndicator`/`HeardTicker` as search). On each natural PAUSE an energy VAD (`dhikr_counter_state.dart`, tunable consts `_speakRms`/`_endSilenceSamples`/`_maxSegSamples`) flushes the spoken segment, transcribes it with the SHARED `WordAsr` (FastConformer — same model that "was catching it very well"), and `countDhikr()` (`services/dhikr/dhikr_match.dart`, pure + 7 tests) increments the matching phrase. Handles in-segment repeats + a fused conjunction wāw (والحمد). "Heard" line shows the transcript. Segment-on-pause (NOT whole-buffer re-transcribe like search) keeps a minutes-long session real-time.
- **Tap-to-count kept; caps removed (33/10 → unlimited); counts PERSIST** (`prefs.dhikrCounts` JSON map); Reset button clears.
- **Model coordination:** reuses the ONE shared `WordAsr`+mic; `DhikrCounterState.cancel()` on tab-change/background frees the model + invalidates the phoneme engine (same two-model handoff as `VoiceSearchState`). Independent review confirmed lifecycle sound, no double-dispose over the shared model.
- **Known v1 accuracy note (device-tune):** during *gap-less* chanting >8s the phrase straddling the `_maxSegSamples` cap is lost (~1/8s); natural pauses avoid it. Raise the cap or lean on pauses.

**⚠️ DEVICE-TEST GATE (unchanged, still owed — everything session 9/10/11 is HOST-tested only):** build + recite on device:
0. **NEW — Counter tab:** open it; tap cards → counts climb, unlimited, survive an app restart. Tap the mic and recite سبحان الله ×N, pause, الحمد لله ×N — the "heard" line shows what it caught and the right cards increment. Leaving the tab frees the mic. Tune the VAD consts if it mis-segments.
1. **Long hadith (100-189 words)** follow-along survives without a permanent freeze (session-10 stall recovery, `eff0a4c`). If it walls, pull the log — `SKIP-ADVANCE past wN` / `SKIP wN frac=` traces show whether recovery fired.
2. **Voice search live-narrows and the ring doesn't flicker** while reciting correctly (commit-horizon `952e590` + the desync fix here). Recite a known verse/hadith; the transcript should grow monotonically, results narrow, ring stay steady.
3. **Restart follow-along** in the Quran reader after a stalled attempt — the previous red skipped-word washes must be GONE on restart (this session's `_asrSkipped` fix).
4. Voice open on all 3 tabs → mic ON + follow-along; tap/typed open → mic OFF.
5. (If you hit it) the backward false-green on a repeated phrase — note it, it's a known deferred limitation, NOT a regression.

## SESSION 10 (2026-07-19) — ENDGAME CLEANUP (A1+A2 DONE)
